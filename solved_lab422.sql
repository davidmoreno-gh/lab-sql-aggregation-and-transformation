...
SELECT 
    ...
    ROUND(AVG(lemgth) % 60) AS avg_minutes
FROM film;
...
/*2.3 Bonus: Retrieve rental information and add an additional column called DAY_TYPE 
...
/*Bonus: The marketing team for the movie rental company now needs to create a 
personalized email campaign for customers. To achieve this, you need to retrieve 
the concatenated first and last names of customers, along with the first 3 
characters of their email address, so that you can address them by their first 
name and use their email address to send personalized recommendations. 
The results should be ordered by last name in ascending order to make it easier 
to use the data.*/
...
order of the number of films. This will help you to better understand 
...
SELECT rating, ROUND(AVG(duration), 2) AS avg_duration
FROM film
GROUP BY rating
